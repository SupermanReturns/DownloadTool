...
- (void)download:(NSString *)url progress:(DownloadProgressBlock)progressBlock state:(DownloadStateBlock)stateBlock
{
    ...
    if (![fileManager fileExistsAtPath:FileFullpath(url)]) {
        ...
        sessionModel.fileName = FileName(url);
...
- (BOOL)isFileDownloadingForUrl:(NSString *)url withProgressBlock:(DownloadProgressBlock)progressBlock
{
    BOOL retValue = NO;
    NSURLSessionDataTask *task = [self getTask:url];
    SessionModel *session = [self getSessionModel:task.taskIdentifier];
    if (session) {
        if (progressBlock) {
...
- (NSArray *)currentDownloads {
    ...
    [self.sessionModels enumerateKeysAndObjectsUsingBlock:^(id key, SessionModel *download, BOOL *stop) {
